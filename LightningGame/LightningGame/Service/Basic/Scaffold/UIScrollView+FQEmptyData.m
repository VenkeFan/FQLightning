...
#define kEmptyScrollViewLightFontColor                     kUIColorFromRGB(0xC4C4C4)
...
- (instancetype)initWithFrame:(CGRect)frame {
    if (self = [super initWithFrame:frame]) {
        ...
        _descLabel.font = kRegularFont(kNameFontSize);
        _descLabel.textColor = kEmptyScrollViewLightFontColor;
        [_contentView addSubview:_descLabel];
        
        _button = [UIButton buttonWithType:UIButtonTypeCustom];
        _button.backgroundColor = kMainBgColor;
        [_button setTitleColor:[UIColor whiteColor] forState:UIControlStateNormal];
        _button.titleLabel.font = kRegularFont(kNameFontSize);
...
- (UIView *)loadingView {
    if (!_loadingView) {
        CGFloat size = 28;
        _loadingView = [[UIView alloc] initWithFrame:CGRectMake(CGRectGetWidth(self.bounds) * 0.5 - size * 0.5,
                                                                              self.contentTop ? self.contentTop + size * 0.5
                                                                              : CGRectGetHeight(self.bounds) * 0.5 - size * 0.5,
                                                                              size,
                                                                              size)];
        
//        _loadingView = [[FQDynamicLoadingView alloc] initWithFrame:CGRectMake(0, 0, size, size)];
...
- (void)reloadEmptyData {
    ...
    if ([self p_ItemsCount] > 0 && self.emptyType != FQScrollEmptyType_Empty_Topic) {
    ...
    if ([self.emptyDataSource respondsToSelector:@selector(descriptionForEmptyDataSource:)]) {
        ...
        if (text.length == 0 && self.emptyType == FQScrollEmptyType_Empty_Network) {
...
- (UIImage *)p_emptyImage {
    ...
    switch (self.emptyType) {
        case FQScrollEmptyType_None:
            imageName = nil;
            break;
        case FQScrollEmptyType_Empty_Data:
            imageName = @"common_empty_data";
            break;
        case FQScrollEmptyType_Empty_Relationship:
            imageName = @"common_empty_relationship";
            break;
        case FQScrollEmptyType_Empty_Message:
            imageName = @"common_empty_message";
            break;
        case FQScrollEmptyType_Empty_Deleted:
            imageName = @"common_empty_deleted";
            break;
        case FQScrollEmptyType_Empty_Network:
            imageName = @"common_empty_network";
            break;
        case FQScrollEmptyType_Empty_Location:
            imageName = @"common_empty_location";
            break;
            
            
        case FQScrollEmptyType_Empty_Topic:
            imageName = nil;
            break;
    }
...
- (void)setEmptyType:(FQScrollEmptyType)emptyType {
...
- (FQScrollEmptyType)emptyType {
    return (FQScrollEmptyType)[objc_getAssociatedObject(self, @selector(emptyType)) integerValue];
}
